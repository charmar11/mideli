    ...
    <div className="space-y-4">
      ...
      <div className="grid gap-4 lg:grid-cols-[1.35fr_1fr]">
        ...
        <Panel className="p-5">
          ...
          <button type="button" onClick={() => onOpen("inbox")} className="mt-4 flex min-h-16 w-full touch-manipulation items-center gap-3 rounded-xl bg-warning/10 p-4 text-left text-warning transition-colors hover:bg-warning/15 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-warning focus-visible:ring-inset active:scale-[0.99]">
            <HandHelping size={20} />
          ...
          <button type="button" onClick={() => onOpen("catalog")} className="mt-2 flex min-h-16 w-full touch-manipulation items-center gap-3 rounded-xl bg-background p-4 text-left transition-colors hover:bg-surface-raised focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand focus-visible:ring-inset active:scale-[0.99]">
...
function Catalog({ data, admin, onRefresh }: { data: WhatsappControlData; admin: boolean; onRefresh: () => void }) {
  ...
  return <Panel><div className="flex flex-col gap-3 border-b border-border p-4 sm:flex-row sm:items-center sm:justify-between"><div><h2 className="font-heading text-lg font-bold">Catálogo de WhatsApp</h2><p className="font-body text-xs text-muted-foreground">Ocultar aquí no elimina el producto del POS.</p></div><label className="relative w-full sm:w-80"><Search className="absolute left-3 top-1/2 -translate-y-1/2 text-muted-foreground" size={16} /><Input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Buscar producto" className="h-11 pl-10" /></label></div><div className="divide-y divide-border">{filtered.map((item) => <div key={item.id} className="flex items-center gap-3 p-4"><div className="min-w-0 flex-1"><p className="font-heading text-sm font-bold">{item.name}</p><p className="font-body text-xs text-muted-foreground">{item.categoryName}{!item.isActive ? " · Inactivo en el menú" : ""}</p></div><label className={`flex min-h-11 touch-manipulation items-center gap-2 rounded-lg px-2 font-heading text-xs ${admin && item.isActive ? "cursor-pointer" : "opacity-50"}`}><input type="checkbox" checked={item.whatsappEnabled && item.isActive} disabled={!admin || !item.isActive || pending} onChange={(event) => toggle(item.id, event.target.checked)} className="h-5 w-5 accent-brand focus-visible:ring-2 focus-visible:ring-brand" />{item.whatsappEnabled && item.isActive ? "Disponible" : "Oculto"}</label></div>)}</div></Panel>;
}
  ...
  return (
    <div className="mx-auto grid max-w-6xl gap-4 xl:grid-cols-[1.15fr_0.85fr]">
      <Panel>
        ...
        <div className="divide-y divide-border">{hours.map((item, index) => <div key={item.dayOfWeek} className="grid items-center gap-3 p-4 sm:grid-cols-[9rem_7rem_1fr]"><strong className="font-heading text-sm">{DAY_NAMES[item.dayOfWeek]}</strong><label className="flex min-h-11 touch-manipulation items-center gap-2 rounded-lg px-1 font-body text-xs"><input type="checkbox" checked={item.isOpen} disabled={!admin} onChange={(event) => setHours((current) => current.map((row, rowIndex) => rowIndex === index ? { ...row, isOpen: event.target.checked } : row))} className="h-5 w-5 accent-brand focus-visible:ring-2 focus-visible:ring-brand" />{item.isOpen ? "Abierto" : "Cerrado"}</label><div className="flex items-center gap-2"><Input type="time" value={item.opensAt} disabled={!admin || !item.isOpen} onChange={(event) => setHours((current) => current.map((row, rowIndex) => rowIndex === index ? { ...row, opensAt: event.target.value } : row))} className="h-11 min-w-0 flex-1" /><span className="shrink-0 text-muted-foreground">a</span><Input type="time" value={item.closesAt} disabled={!admin || !item.isOpen} onChange={(event) => setHours((current) => current.map((row, rowIndex) => rowIndex === index ? { ...row, closesAt: event.target.value } : row))} className="h-11 min-w-0 flex-1" /></div></div>)}</div>
      ...
      <div className="space-y-4">
        <Panel className="p-4">
          ...
          <label className="mt-3 flex min-h-11 touch-manipulation items-center gap-2 rounded-lg px-1 font-body text-sm"><input type="checkbox" checked={exception.isOpen} disabled={!admin} onChange={(event) => setException((current) => ({ ...current, isOpen: event.target.checked }))} className="h-5 w-5 accent-brand focus-visible:ring-2 focus-visible:ring-brand" />Abrir con horario especial</label>
