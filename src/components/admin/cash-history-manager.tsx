  ...
  return (
    <div className="min-h-dvh min-w-0 max-w-full overflow-x-hidden bg-background text-foreground">
      ...
      <nav className="pos-scroll mx-auto flex max-w-[1500px] touch-pan-x gap-2 overflow-x-auto overscroll-contain px-3 pt-3 sm:px-5 lg:px-6 print:hidden" aria-label="Secciones de caja">
        <button type="button" onClick={() => setActiveTab("shifts")} aria-pressed={activeTab === "shifts"} className={`min-h-11 shrink-0 touch-manipulation rounded-xl px-4 py-3 font-heading text-sm font-bold transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand focus-visible:ring-inset ${activeTab === "shifts" ? "bg-brand text-white" : "bg-surface text-muted-foreground hover:bg-surface-raised hover:text-foreground"}`}>Turnos y cortes</button>
      ...
      {activeTab === "movements" ? <CashMovementsManager /> : <main className="mx-auto max-w-[1500px] p-3 sm:p-5 lg:p-6">
        ...
        <div className="grid min-h-[620px] min-w-0 overflow-hidden rounded-2xl border border-border bg-surface lg:grid-cols-[390px_minmax(0,1fr)]">
            ...
            <div className="space-y-3 border-b border-border p-3">
              ...
              <div className="grid grid-cols-4 rounded-xl bg-background p-1">
                {(["all", "open", "closed", "archived"] as const).map((value) => (
                  <button
                    ...
                    className={`min-h-11 touch-manipulation rounded-lg font-heading text-[11px] font-bold focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand focus-visible:ring-inset ${status === value ? "bg-brand text-white" : "text-muted-foreground"}`}
                  >
              ...
              <div className="grid grid-cols-2 gap-2">
                <label className="min-w-0">
                  ...
                  <select value={period} onChange={(event) => setPeriod(event.target.value as typeof period)} className="h-11 w-full min-w-0 rounded-xl border border-border bg-background px-2 font-heading text-xs font-bold outline-none focus:border-brand focus-visible:ring-2 focus-visible:ring-brand">
                ...
                <label className="min-w-0">
                  ...
                  <select value={attention} onChange={(event) => setAttention(event.target.value as typeof attention)} className="h-11 w-full min-w-0 rounded-xl border border-border bg-background px-2 font-heading text-xs font-bold outline-none focus:border-brand focus-visible:ring-2 focus-visible:ring-brand">
              ...
              <label className="block">
                ...
                <select value={responsible} onChange={(event) => setResponsible(event.target.value)} className="h-11 w-full rounded-xl border border-border bg-background px-3 font-heading text-xs font-bold outline-none focus:border-brand focus-visible:ring-2 focus-visible:ring-brand">
            ...
            {detailLoading ? <div className="flex flex-1 items-center justify-center"><Loader2 className="animate-spin text-brand" /></div> : selected ? (
              <>
                <div className="flex flex-wrap items-center gap-2 border-b border-border p-4 print:border-black print:bg-white print:text-black">
                  <button type="button" onClick={() => setSelected(null)} className="flex h-11 w-11 items-center justify-center rounded-xl bg-surface-raised focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand lg:hidden print:hidden"><ArrowLeft size={17} /></button>
                  ...
                  {selected.status === "closed" ? <button type="button" onClick={() => window.print()} className="flex min-h-11 items-center gap-2 rounded-xl bg-surface-raised px-3 font-heading text-xs font-bold focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand print:hidden"><Printer size={15} />Imprimir</button> : null}
                  {selected.status === "closed" ? <button type="button" onClick={() => void shareSelected()} className="flex min-h-11 items-center gap-2 rounded-xl bg-surface-raised px-3 font-heading text-xs font-bold focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand print:hidden"><Share2 size={15} /><span className="hidden sm:inline">Compartir</span></button> : null}
                  {selected.status === "open" ? <button type="button" onClick={openOpeningFloatCorrection} className="flex min-h-11 items-center gap-2 rounded-xl bg-gold/12 px-3 font-heading text-xs font-bold text-gold focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-gold print:hidden"><Pencil size={15} /><span className="hidden sm:inline">Corregir fondo</span></button> : null}
                  {selected.status === "closed" && !selected.archived_at ? <button type="button" onClick={() => void openAdjustment()} className="flex min-h-11 items-center gap-2 rounded-xl bg-warning/12 px-3 font-heading text-xs font-bold text-warning focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-warning print:hidden"><RotateCcw size={15} /><span className="hidden sm:inline">Corregir</span></button> : null}
                  {selected.status === "closed" && !selected.archived_at ? <button type="button" aria-label="Archivar corte" onClick={openArchiveDialog} className="flex min-h-11 items-center gap-2 rounded-xl bg-warning/12 px-3 font-heading text-xs font-bold text-warning focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-warning print:hidden"><Trash2 size={15} /><span className="hidden sm:inline">Archivar</span></button> : null}
                  {selected.archived_at ? <button type="button" onClick={() => setRestoreDialog(true)} className="action-success flex min-h-11 items-center gap-2 rounded-xl px-3 font-heading text-xs font-bold focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-success print:hidden"><ArchiveRestore size={15} />Restaurar</button> : null}
                  {selected.archived_at ? <button type="button" onClick={() => void openPermanentDeleteDialog()} className="flex min-h-11 items-center gap-2 rounded-xl bg-destructive px-3 font-heading text-xs font-bold text-white transition-colors hover:bg-destructive/85 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-destructive print:hidden"><Trash2 size={15} /><span className="hidden sm:inline">Eliminar definitivamente</span></button> : null}
                </div>
      ...
      {adjusting && selected ? <div className="fixed inset-0 z-50 flex items-end justify-center bg-ink/75 sm:items-center sm:p-4"><section role="dialog" aria-modal="true" aria-labelledby="adjust-shift-title" className="max-h-[calc(100dvh-1rem)] w-full max-w-lg overflow-y-auto rounded-t-2xl border border-border bg-surface p-4 shadow-float overscroll-contain sm:rounded-2xl sm:p-5"><div className="mb-4 flex items-center justify-between"><div><h2 id="adjust-shift-title" className="font-heading text-lg font-black">Corregir corte #{selected.number}</h2><p className="font-body text-xs text-muted-foreground">Se agrega un registro; el corte original no cambia.</p></div><button type="button" onClick={() => setAdjusting(false)} aria-label="Cerrar" className="flex h-11 w-11 items-center justify-center rounded-xl text-muted-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-brand hover:bg-surface-raised"><X size={18} /></button></div><div className="space-y-3"><div className="grid grid-cols-2 gap-2"><select value={adjustMethod} onChange={(event) => setAdjustMethod(event.target.value as typeof adjustMethod)} className="h-11 rounded-xl border border-border bg-background px-3 font-body text-sm"><option value="efectivo">Efectivo</option><option value="tarjeta">Tarjeta</option><option value="transferencia">Transferencia</option><option value="otro">Otro</option></select><select value={adjustDirection} onChange={(event) => setAdjustDirection(event.target.value as typeof adjustDirection)} className="h-11 rounded-xl border border-border bg-background px-3 font-body text-sm"><option value="increase">Sumar</option><option value="decrease">Restar</option></select></div><input type="number" inputMode="decimal" min="0" step="0.01" value={adjustAmount || ""} onChange={(event) => setAdjustAmount(Number(event.target.value))} placeholder="Importe" className="h-12 w-full rounded-xl border border-border bg-background px-3 font-data text-lg font-bold" /><textarea value={adjustReason} onChange={(event) => setAdjustReason(event.target.value)} placeholder="Motivo de la corrección" rows={2} className="w-full resize-none rounded-xl border border-border bg-background p-3 font-body text-sm" /><div className="rounded-xl border border-warning/30 bg-warning/8 p-3"><div className="mb-2 flex items-center gap-2 font-heading text-sm font-bold"><LockKeyhole size={16} className="text-warning" />Autorización</div><div className="grid gap-2 sm:grid-cols-2"><select value={authorizerId} onChange={(event) => setAuthorizerId(event.target.value)} className="h-11 rounded-xl border border-border bg-background px-3 font-body text-sm"><option value="">Responsable</option>{authorizers.map((authorizer) => <option key={authorizer.id} value={authorizer.id} disabled={!authorizer.pin_configured}>{authorizer.full_name}{authorizer.pin_configured ? "" : " · sin PIN"}</option>)}</select><input value={pin} onChange={(event) => setPin(event.target.value.replace(/\D/g, "").slice(0, 4))} type="password" inputMode="numeric" placeholder="PIN" className="h-11 rounded-xl border border-border bg-background px-3 font-data tracking-[0.3em]" /></div></div><button type="button" disabled={saving} onClick={() => void saveAdjustment()} className="action-success inline-flex h-12 w-full items-center justify-center gap-2 rounded-xl font-heading text-sm font-bold focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-success disabled:opacity-50">{saving ? <Loader2 size={17} className="animate-spin" /> : <CheckCircle2 size={17} />}Autorizar y registrar</button></div></section></div> : null}
